...
if data is not None:
    ...
    # Imputation
    imputation.find_missing_data(data)
    imputed_data = imputation.spline(data)
    imputation.find_missing_index(data)
    st.subheader('Timeseries with filled missing data', divider=True)
    fig = pb.plot_scatter(imputed_data, columns_list=['Raw_Data'])
    ...
    #     model.fit_predict(data=imputed_data)
    #     model.anomalies()
    #
