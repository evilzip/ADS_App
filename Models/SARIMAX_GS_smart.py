...
class SARIMAX_GS_smart:
    ...
    def anomalies(self):
        ...
        self.model_df_least_MAPE['Anomalies'] = (self.model_df_least_MAPE['Raw_Data'] < self.model_df_least_MAPE[
            'Lower_CI']) | (
                                                    ...
                                                    'Upper_CI'])
        anomalies = self.model_df_least_MAPE[['Time', 'Anomalies']]
        anomalies = anomalies[anomalies['Anomalies'] == True]
        return anomalies

    def smart_fit(self, data):
        # 1. fill up model result dataframe
        self.model_df_least_MAPE['Time'] = \
            self.model_df_least_MSE['Time'] = \
            self.model_df_least_AIC['Time'] = \
            self.model_df_least_BIC['Time'] = data['Time']
        self.model_df_least_MAPE['Raw_Data'] = \
            self.model_df_least_MSE['Raw_Data'] = \
            self.model_df_least_AIC['Raw_Data'] = \
            self.model_df_least_BIC['Raw_Data'] = data['Raw_Data']
        self.model_df['Time'] = data['Time']
        self.model_df['Raw_Data'] = data['Raw_Data']
        raw_data = data['Raw_Data'].values.tolist()  # ? ДЛя чего ?

        # 2. Train test split
        train, test = self.train_test_split(data, k=0.9)
        print('train/test', train, test)
        self.train, self.test = train, test

        # 3. Find dominant period = main season = m
        dominant_period, _, _ = self.fft_analysis(data['Raw_Data'].values)
    ...
    def _model_quality(self):
        ...
        mape = mean_absolute_percentage_error(self.test,
                                              self.model_df_least_MAPE['Y_Predicted'].iloc[self.test.index[0]:])
        print('mape data', self.test, self.model_df_least_MAPE['Y_Predicted'].iloc[self.test.index[0]:])
...
#
# # data
# data = pd.read_csv('C:\DATA\ADS_App\Data\daily-total-female-births-in-cal.csv')
# data = data.rename(columns={data.columns[0]: 'Time',
#                             data.columns[1]: 'Raw_Data'})
# print(data)

# fit
# sarimax_gs_smart.smart_fit(data)
# print(sarimax_gs_smart.best_models)
#
# # # create train-test-split
# # train = data['Count'].iloc[:int(len(data) * 0.9)]
# # test = data['Count'].iloc[int(len(data) * 0.9):]
# #
# # # possible values
# # p = [1, 2, 3]
# # d = [0, 1]
...
# #
# # model_info = sarimax_gs_smart.SARIMA_grid(endog=train, order=pdq, seasonal_order=PDQm)
# #
# # end = time.time()
